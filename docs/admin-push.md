...
5. Open `/lisa/admin/` or `/liyong/admin/` on the owner's device and log in. Tap the crossed-out bell next to Buchungslog to enable notifications, then accept the browser's prompt. On iPhone/iPad, first add each PWA to the Home Screen and open it from there. Tap the bell again to disable notifications on that device; tap it later to re-enable them.
